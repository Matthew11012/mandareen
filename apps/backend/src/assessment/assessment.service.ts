...
export class AssessmentService {
  ...
  async fetchAssessmentQuestions(userId: number, dto: FetchQuestionsDto): Promise<Passage[]> {
    try {
      ...
      const levelDistribution = this.calculateLevelDistribution(maxLevel, passageCount);
  ...
  async submitAssessment(userId: number, dto: SubmitAssessmentDto): Promise<{ levelPlaced: number }> {
    try {
  ...
  private calculatePlacementLevel(wordAssessments: SubmitAssessmentDto['wordAssessments']): number {
    const wordsByLevel = new Map<number, { unknown: number, partial: number, total: number }>();
    ...
    for (let level = 1; level <= 7; level++) {
      ...
      const weightedUnknownRatio = (stats.unknown + PARTIAL_WEIGHT * stats.partial) / stats.total;
  ...
  private calculateLevelDistribution(maxLevel: number, passageCount: number): number[] {
