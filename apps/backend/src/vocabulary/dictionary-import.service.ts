...
export class DictionaryImportService {
  ...
  private async loadAndApplyHSKLevels(): Promise<void> {
    try {
      ...
      if (!fs.existsSync(hskDataPath)) {
        this.logger.warn(
          'HSK levels file not found, skipping HSK level assignment',
        );
        return;
      }
  ...
  async importCCCEDICT(filePath: string): Promise<void> {
    ...
    this.logger.log(
      `Import completed. Total entries: ${totalProcessed}, Skipped lines: ${skippedLines}`,
    );
  }
  ...
  async importHSKLevels(
    ...
    this.logger.log(
      `HSK level assignment completed. Updated ${updated} words.`,
    );
  }
  ...
  async addSampleData(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Added ${sampleVocabulary.length} sample vocabulary items`,
      );
    } catch (error) {
  ...
  async getDictionaryStats() {
    const [totalWords, customWords, hskWords, levelDistribution] =
      await Promise.all([
